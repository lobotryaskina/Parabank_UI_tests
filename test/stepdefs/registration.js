...
When('I fill in the account information with fake data with an unmatching password', async () => {
  fakeUser = new FakeUser();
  user = fakeUser.makeUser();
  await registerPage.fillFirstName(user.firstName);
  await registerPage.fillLastName(user.lastName);
...
When('I fill in the account information with fake data and an existing login', async () => {
  fakeUser = new FakeUser();
  user = fakeUser.makeUser();
  await registerPage.fillFirstName(user.firstName);
  await registerPage.fillLastName(user.lastName);
...
Then('I should see a not-matching password error message', async () => {
  await registerPage.checkLocatorExists(registerPage.locators.matchingPassswordError);
});

Then('I should see an existing user error message', async () => {
  await registerPage.checkLocatorExists(registerPage.locators.existingUserError);
});